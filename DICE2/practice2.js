...
let current_score = 0;
// let active_player = 0;
let active_player=Math.floor(Math.random()*2);
let score = [0, 0]
...
hold_btn.addEventListener('click', function () {
    if (play) {
        ...
        if (score[active_player] >= 20) {
            ...
            document.querySelector(`.player--${active_player}`).style.backgroundColor = "green";
            document.querySelector(`.player--${active_player === 0 ? activePlayer = 1 : activePlayer = 0}`).style.backgroundColor = "black";
            document.querySelector(`#score--${active_player}`).innerHTML = "Winner";
            document.querySelector(`#score--${active_player === 0 ? activePlayer = 1 : activePlayer = 0}`).innerHTML = "Looser";
            // current_score_player1.style.color="green";
            play = false;
            dice.classList.add('hidden');
        }
        else {
...
let ChangeTurn = function () {
    current_score = 0;
    document.getElementById(`current--${active_player}`).innerHTML = 0;
    player1.classList.toggle('player--active'); //toggle method removes if class is added and vice versa
    player2.classList.toggle('player--active');
    (active_player === 0 ? active_player = 1 : active_player = 0)
}

newgame.addEventListener('click', function () {
    document.querySelector(`.player--${active_player}`).style.backgroundColor = "";
    document.querySelector(`.player--${active_player === 0 ? activePlayer = 1 : activePlayer = 0}`).style.backgroundColor = "";
